...
The 4-deep ring smooths over any single SDRAM stall up to ~24 µs. SDRAM auto-refresh is ~70 cycles every 7.8 µs at 100 MHz = 700 ns of stall every 7.8 µs = 9% overhead in the worst case, which is well under the cache depth.

**Read trigger:** during H-blank of line N, kick a burst read for line N+2 into whichever cache slot is free. Burst reads are sequential and the controller can run them open-page if rows are aligned to whole lines (1600 bytes < typical SDRAM row size of 4 KB, so each line is one page open + burst).
...
|---|---|---|---|
| FB_FRONT | 0x0000_0000 – 0x000B_BFFF | 768 KB | active framebuffer (RGB565, 800×480) |
| FB_BACK | 0x000C_0000 – 0x0017_BFFF | 768 KB | back buffer; swap pointer at V-sync |
| WATERFALL_HISTORY | 0x0018_0000 – 0x001F_FFFF | 512 KB | extra waterfall depth (scrollback) |
| GOES_FULL_IMAGE | 0x0020_0000 – 0x0027_FFFF | 512 KB | full GOES image at 909×... if we go beyond strip |
| ADSB_BASEMAP | 0x0028_0000 – 0x002B_FFFF | 256 KB | static Riverside map image (800×448 RGB565 ≈ 700 KB if full-res; tile/scale to fit) |
| AUDIO_RING | 0x002C_0000 – 0x002F_FFFF | 256 KB | optional audio history for replay |
| SCRATCH | 0x0030_0000 – 0x01FF_FFFF | ~29 MB | unallocated; future use |

The ECP5 only needs to know about FB_FRONT/FB_BACK plus the scratch waterfall/GOES/ADS-B regions for compositor writes. Everything else is "we have it if we need it." The ADS-B map is static, so the compositor can blit it from ADSB_BASEMAP once on mode entry and then only redraw plane dots.

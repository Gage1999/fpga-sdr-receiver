module i2s_tx #(
    ...
    parameter int AUDIO_RATE = 43_625,   // LRCLK target; NCO hits this fractionally
    parameter int SLOT_BITS = 32,
    parameter int BCLK_HALF_CYCLES = 0    // 0 = fractional NCO, otherwise jitter-free integer divider
)(
    input  logic clk,
    input  logic rst,
...
// BCLK generator. By default it uses a fractional NCO to hit arbitrary audio
// rates; set BCLK_HALF_CYCLES for a jitter-free integer divider when the DAC is
// deriving its clock from BCLK.
...
// bclk_fall: one-cycle pulse on each falling BCLK edge.
// bclk_tick and bclk are both registered, so after the clock edge that toggles
// BCLK low: bclk_tick=1, bclk=0 (already updated). Detected the next cycle.
...
// Serializer: advances on each falling BCLK edge.
// sdata updates one CLK cycle after bclk_fall, stable 2+ cycles before the
// next BCLK rising edge (avg 4.5 CLK cycles per half-period -> ample margin).
always_ff @(posedge clk or posedge rst) begin
    if (rst) begin
        bit_cnt    <= '0;
        lrclk      <= 1'b0;

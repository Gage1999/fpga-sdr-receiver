// sdram_arb.sv
// 3-client fixed-priority arbiter for sdram_ctrl (0=scan_out > 1=compositor > 2=ingest).
// No preemption: a grant is held until that transaction's `done`, then re-arbitrated.
// Client ports are packed vectors (lane i = bit i, or word slice [i*W +: W]).
// See docs/fpga-sdr-receiver-architecture.md §3.
...
module sdram_arb #(
    ...
    // Highest-priority pending requester (lowest index wins)
    logic       pick_valid;
    logic [1:0] pick;
    always_comb begin
        pick_valid = 1'b0;
    ...
    // Grant register: held from grant until the transaction's done pulse
    ...
    always_comb begin
        ...
        c_done      = '0;
        for (int i = 0; i < N; i++)
            c_rd_data[i*16 +: 16] = m_rd_data;   // broadcast read data
        if (gnt_valid) begin
